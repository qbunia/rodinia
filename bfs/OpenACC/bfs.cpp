...
#pragma acc data copyin(                                                       \
    no_of_nodes, h_graph_mask [0:no_of_nodes], h_graph_nodes [0:no_of_nodes],  \
    h_graph_edges [0:edge_list_size], h_graph_visited [0:no_of_nodes],         \
    h_updating_graph_mask [0:no_of_nodes]) copy(h_cost [0:no_of_nodes])
  {
    bool stop;
    do {
...
#pragma acc parallel loop num_gangs(omp_num_teams) num_workers(1)              \
    vector_length(omp_num_threads)
...
#pragma acc parallel loop copy(stop) num_gangs(omp_num_teams) num_workers(1)   \
    vector_length(omp_num_threads)
      for (int tid = 0; tid < no_of_nodes; tid++) {
