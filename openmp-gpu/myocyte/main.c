...
int main(int argc, char *argv []){
	...
	y[0] = (fp **) malloc(workload * (1+xmax) * sizeof(fp*));
	y[0][0] = (fp *) malloc(workload * (1+xmax) * EQUATIONS * sizeof(fp));
	for (i= 1; i<workload; i++){
		y[i] = y[i-1] + (1+xmax);
		for (j= 0; j<(1+xmax); j++){
			if (i!=0 && j!=0)
				y[i][j] = y[i][j-1] + ((1+xmax) * EQUATIONS);
		}
	}

	x = (fp **) malloc(workload * sizeof(fp *));
	x[0] = (fp *) malloc(workload * (1+xmax) * sizeof(fp));
	for (i= 1; i<workload; i++){
		x[i] = x[i-1] + (1+xmax);
	}

	params = (fp **) malloc(workload * sizeof(fp *));
	params[0] = (fp *) malloc(workload * PARAMETERS * sizeof(fp));
	for (i= 1; i<workload; i++){
		params[i] = params[i-1] + PARAMETERS;
	}

	err = (fp **) malloc(workload * sizeof(fp *));
	err[0] = (fp *) maaloc(workload * EQUATIONS * sizeof(fp));
	for (i= 1; i<workload; i++){
		err[i] = err[i-1] + EQUATIONS;
	}

	scale = (fp **) malloc(workload * sizeof(fp *));
	scale[0] = (fp *) maaloc(workload * EQUATIONS * sizeof(fp));
	for (i= 1; i<workload; i++){
		scale[i] = scale[i-1] + EQUATIONS;
	}

	yy = (fp **) malloc(workload * sizeof(fp *));
	yy[0] = (fp *) maaloc(workload * EQUATIONS * sizeof(fp));
	for (i= 1; i<workload; i++){
