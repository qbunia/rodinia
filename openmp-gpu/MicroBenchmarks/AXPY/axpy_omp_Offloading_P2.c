...
void axpy_omp_offloading_P2(REAL* x, REAL* y, long n, REAL a) {
  #pragma omp target teams distribute parallel for map(to: n, x[0:n]) map(tofrom: y[0:n]) num_teams(TEAM_NUM) num_threads(TEAM_SIZE) schedule(static, 256)
  for (long i = 0; i < n; ++i) {
    y[i] += a * x[i];
  }
}
...
int main(int argc, char *argv[])
{
  ...
  //warm up
  axpy_omp_offloading_P2(x, y, n, a);

  double elapsed = read_timer_ms();
  for (i=0; i<num_runs; i++) axpy_omp_offloading_P2(x, y, n, a);
