...
void dump(double *variables, int nel, int nelr) {
  ...
  {
    std::ofstream file("momentum.log");
    file << nel << " " << nelr << std::endl;
    for (int i = 0; i < nel; i++) {
  ...
  {
    std::ofstream file("density_energy.log");
    file << nel << " " << nelr << std::endl;
    for (int i = 0; i < nel; i++)
...
#pragma acc parallel loop gang num_gangs(NUM_TEAMS) num_workers(1)             \
    vector_length(NUM_THREADS)                                                 \
        present(elements_surrounding_elements, normals, variables, fluxes)
  for (int i = 0; i < nelr; i++) {
    int j, nb;
    double3 normal;
    double normal_len;
