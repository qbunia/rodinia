...
void matvec_omp_offloading_P2(int N, REAL *A, REAL *B, REAL *C) {
    int i, j, k;
    int size = N * N;

    #pragma omp target data map(to: A[0:size], B[0:N]) map(from: C[0:N])
    {
        #pragma omp target teams distribute parallel for num_teams(TEAM_NUM) num_threads(TEAM_SIZE)
        for (i = 0; i < N; i++) {
            REAL temp = 0.0;
            #pragma unroll(4) 
...
int main(int argc, char *argv[]) {
    ...
    //warm up
    matvec_omp_offloading_P2(N, A, B, C_omp);
