...
#pragma acc parallel loop private(i, thid) num_gangs(NUM_TEAMS) num_workers(1) \
    vector_length(NUM_THREADS) copyin(currKnode [0:count], offset [0:count])   \
        copyin(knodes [0:knodes_elem], keys [0:count]) copy(ans [0:count])     \
            present(records)
