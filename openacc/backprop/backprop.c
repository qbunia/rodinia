...
void bpnn_output_error(float *delta, float *target, float *output, int nj,
  ...
  float errsum = 0.0;
#pragma acc parallel loop num_gangs(NUM_TEAMS) num_workers(1)                  \
    vector_length(NUM_THREADS) present(delta, target, output)                  \
    reduction(+ : errsum)
  for (int j = 1; j <= nj; j++) {
    float o = output[j];
    float t = target[j];
...
void bpnn_hidden_error(float *delta_h, int nh, float *delta_o, int no,
  ...
  float errsum = 0.0;
#pragma acc parallel loop num_gangs(NUM_TEAMS) num_workers(1)                  \
    vector_length(NUM_THREADS) present(delta_h, delta_o, who, hidden)          \
    reduction(+ : errsum)
  for (int j = 1; j <= nh; j++) {
    float h = hidden[j];
    float sum = 0.0;
...
void bpnn_adjust_weights(float *delta, int ndelta, float *ly, int nly, float *w,
                         float *oldw) {
  ly[0] = 1.0;
#pragma acc update device(ly[0])

#pragma acc parallel loop num_gangs(NUM_TEAMS) num_workers(1)                  \
    vector_length(NUM_THREADS) present(delta, ly, w, oldw) collapse(2)
