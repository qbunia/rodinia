...
void run(int argc, char **argv) {
  ...
  pin_stats_reset();
#pragma acc data copy(src [0:cols], dst [0:cols]) copyin(data [0:rows * cols])
  {
    for (int t = 0; t < rows - 1; t++) {
      temp = src;
      src = dst;
      dst = temp;
#pragma acc parallel loop private(min) copy(src [0:cols], dst [0:cols])        \
    copyin(data [0:rows * cols]) num_gangs(NUM_TEAMS) num_workers(1)           \
